...
def read_users_me(current_user: Annotated[User, Depends(get_current_user)]):
    """
    (Com迆n) Obtiene los datos del usuario logueado. 
    迆til para que el frontend sepa el rol y el ID.
    """
    return current_user
...
def change_password(
    ...
    """
    (Com迆n) Permite a un usuario cambiar su contrase?a.
    Requiere la contrase?a actual para validaci車n.
    """
...
def get_all_students(
    ...
    para ser usados en la asignaci車n de rutinas.
    """
...
def get_all_exercises(session: Annotated[Session, Depends(get_session)]):
    """
    (Com迆n) Obtiene una lista de todos los ejercicios.
    """
...
def get_routine_by_id(
    ...
    """(Com迆n) Obtiene una rutina por su ID (usado principalmente por el profesor para editar)."""
...
def create_routine_assignment(
    *, 
    ...
    if not new_assignment:
        raise HTTPException(status_code=500, detail="Error al recuperar la asignaci車n creada.")
...
def get_student_assignments(
    ...
    """
    (Profesor) Obtiene todas las asignaciones (activas e inactivas) de un alumno espec赤fico.
    """
...
def toggle_assignment_active(
    ...
    """(Profesor) Activa o desactiva una asignaci車n de rutina."""
...
def delete_assignment(
    ...
    """(Profesor) Elimina una asignaci車n de rutina por su ID."""
    assignment = session.get(RoutineAssignment, assignment_id)

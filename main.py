...
def register_student(
    ...
    if existing_dni:
        raise HTTPException(
            ...
            detail="El DNI ya est芍 registrado."
        )
    ...
    if existing_email:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="El email ya est芍 registrado."
        )

    # ?? NOTA: Se asume que el backend maneja el l赤mite de 72 bytes para el password hash
...
def login_for_access_token(
    ...
    """Verifica credenciales (Email/DNI y Password) y devuelve un JWT para la sesi車n."""
    
    # ?? B迆squeda por EMAIL (asumiendo que el campo 'dni' en el payload del frontend es el Email)
    user = session.exec(select(User).where(User.email == form_data.dni)).first()
    
    # Si la b迆squeda por email falla, intentar por DNI (fallback)
    if not user:
...
def create_routine_group_and_routines(
    ...
    if not data.routines:
        raise HTTPException(status_code=400, detail="La lista de rutinas no puede estar vac赤a.")
    ...
    assignment = RoutineAssignment(
        ...
        professor_id=current_professor.id,
        is_active=True # Se activa autom芍ticamente
    )
    session.add(assignment)
    session.commit()
...
def delete_routine(
    ...
    """Elimina una rutina maestra. Esto tambi谷n eliminar芍 los enlaces en RoutineExercise y asignaciones."""
    db_routine = session.get(Routine, routine_id)
    if not db_routine:
        raise HTTPException(status_code=404, detail="Rutina no encontrada")
...
def assign_routine_to_student(
    ...
    """(Profesor) Asigna una rutina a un alumno. Permite m迆ltiples asignaciones (no desactiva las antiguas)."""
...
def set_assignment_active_status(
    assignment_id: int,
    is_active: bool, # Nuevo par芍metro booleano para el estado
    session: Annotated[Session, Depends(get_session)],
    current_professor: Annotated[User, Depends(get_current_professor)]
):
...
def get_my_active_routine(
    ...
    """
    (Alumno) Obtiene SOLAMENTE las rutinas asignadas que est芍n marcadas como activas (is_active=True).
    """
    statement = (
        select(RoutineAssignment)
        .where(

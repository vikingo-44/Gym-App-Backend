...
    
    # [CAMBIO CLAVE 1] Usamos 'email' como 'sub' (subject) en el token
    # Se asume que 'data' ahora contiene la clave 'email'
...
        # [CAMBIO CLAVE 2] El email esta ahora en el campo 'sub'
...
        raise credentials_exception
    
    # [CAMBIO CLAVE 3] Buscar usuario por Email
...
    form_data: UserLogin, # form_data ahora tiene .email y .password (Se asume que el esquema UserLogin fue modificado)
    session: Annotated[Session, Depends(get_session)]
):
    """Verifica credenciales (Email y Password) y devuelve un JWT para la sesion."""
    
    # [CAMBIO CLAVE 4] Busqueda UNICAMENTE por EMAIL (se usa form_data.email)
    user = session.exec(select(User).where(User.email == form_data.email)).first()
    
    # Eliminado: El fallback a DNI ya no es necesario
...
    # [CAMBIO CLAVE 5] Incluimos el email del usuario en el token.

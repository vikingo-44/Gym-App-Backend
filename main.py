...
    
    # MODIFICACION 1/4: Usamos 'email' como 'sub' (subject) en el token en lugar de 'dni'.
...
        # MODIFICACION 2/4: El email esta ahora en el campo 'sub'
...
        raise credentials_exception
    
    # MODIFICACION 3/4: Buscar usuario por EMAIL (que esta en 'sub')
...
    
    # Busqueda principal por EMAIL (asumiendo que el campo 'dni' en el payload del frontend es el Email)
...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Credenciales (Email/DNI o Contrasena) incorrectas",
            headers={"WWW-Authenticate": "Bearer"},
        )
    
    access_token_expires = timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    # MODIFICACION 4/4: Usamos el email del usuario para crear el token, ademas del rol y nombre.

...
? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? </Text>
? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? ? </Text>
                                             {/* LISTADO DE EJERCICIOS CON PESO */}
                                             <View style={{ marginTop: 10, borderTopWidth: 1, borderTopColor: themeColors.divider, paddingTop: 5 }}>
                                                 <Text style={{ fontSize: 13, fontWeight: 'bold', color: themeColors.textPrimary }}>Ejercicios:</Text>
                                                 {assignment.routine.exercise_links
                                                     .sort((a, b) => a.order - b.order)
                                                     .map((link, exIndex) => (
                                                         <View key={link.id || exIndex} style={{ paddingLeft: 10, paddingVertical: 2 }}>
                                                             <Text style={{ fontSize: 14, fontWeight: '600', color: themeColors.textPrimary }}>
                                                                 {link.order}. {link.exercise.nombre}
                                                             </Text>
                                                             <Text style={{ fontSize: 12, color: themeColors.textSecondary }}>
                                                                 Sets: {link.sets}, Reps: {link.repetitions}
                                                             </Text>
                                                             {/* ?? MODIFICACION CLAVE: VISUALIZACION DEL PESO */}
                                                             <Text style={{ fontSize: 12, color: themeColors.textSecondary, fontWeight: 'bold' }}>
                                                                 Peso: {link.peso || 'N/A'}
                                                             </Text>
                                                         </View>
                                                     ))}
